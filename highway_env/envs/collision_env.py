...
class CollisionEnv(HighwayEnv):
    ...
    def _update_state(self):
        ...
        if self.active == 2:
            if (self.time - self.time_since_avoidance) > self.config["control_time_after_avoid"] * self.config["simulation_frequency"]:
                self.active = 0
    ...
    def _reward(self, action: Action) -> float:
        ...
        """
        duration_reached = self.time >= self.config["duration"] * self.config["simulation_frequency"]
        ...
        reward += self.config["time_alive_reward"] * self.time / self.config["simulation_frequency"]
        reward += self.config["lateral_velocity_reward"] * abs(self.vehicle.lateral_velocity) if self.active == 2 else 0
        reward = self.config["offroad_reward"] if not self.vehicle.on_road else reward
    ...
    def _is_terminal(self) -> bool:
        ...
        """
        if self.vehicle.crashed:
            end_time = self.time + self.config["time_after_collision"] * self.config["simulation_frequency"]
            while self.time < end_time:
                self._simulate(None)

        return self.vehicle.crashed or \
            self.time >= self.config["duration"] * self.config["simulation_frequency"] or \
    ...
    def _info(self, obs: ObservationType, action: Action) -> dict:
        ...
        info = {
            ...
            "time": self.time/self.config["simulation_frequency"],
